function initialAssessmentQAViewModelCustom(model) {
    ...
    self.isAccepted = ko.observable("false");
    ...
    $(document).ready(function () {
        ...
        $('.close').click(function () {
            $("#finalVersionFileUploadFile").replaceWith($("#finalVersionFileUploadFile").clone());
        });
    ...
    self.UpdateInitialAssessment = function (result) {
        ...
        //        if (s == 0) {
        //            alert('Total amount of treatment prices for the case exceed the Delegated Authorization, the case would be sent to Authorization.');
        //        }
        //        else {
    ...
    if (model.CaseTreatmentPricings.length > 0)
    {
        $.each(model.CaseTreatmentPricings, function (index, value) {
            for (var i = 0; i < model.TreatmentReferrerSupplierPricing.length; i++) {
                if (value.ReferrerPricingID === model.TreatmentReferrerSupplierPricing[i].ReferrerPricingID) {
                    self.CaseTreatmentPricing.push(new CaseTreatmentPricingModel(value, model.TreatmentReferrerSupplierPricing[i].PricingTypeName));
