...
//    });
    ...
    //        //self.PsychologicalFactors = ko.observableArray();
    //        self.PsychologicalFactorID(model.CaseAssessment.PsychologicalFactorID);
    ...
    //        self.PatientWorkstatusID(model.CaseAssessment.PatientWorkstatusID);
    //        //self.PatientWorkstatuses = ko.observableArray();
    //        self.PatientImpactOnWorkID(model.CaseAssessment.PatientImpactOnWorkID);
    //        //self.PatientImpactOnWorks = ko.observableArray();
    //        /**end - IMPACT ON WORK**/
    ...
    //        self.AnticipatedDateOfDischarge(moment(model.CaseAssessment.AnticipatedDateOfDischarge).format('MM/DD/YYYY'));
    //        self.PatientLevelOfRecoveryID(model.CaseAssessment.PatientLevelOfRecoveryID);
    //        //self.PatientLevelOfRecoveries = ko.observableArray();
    //        self.HasPatientHomeExerciseProgramme(model.CaseAssessment.HasPatientHomeExerciseProgramme);
    //        self.ProposedTreatmentMethodID(model.CaseAssessment.ProposedTreatmentMethodID);
    //        //self.ProposedTreatmentMethods = ko.observableArray();
    //        self.AdditionalInformation(model.CaseAssessment.AdditionalInformation);
    //        /**end - TREATMENT RECOMMENDATION**/
    //        //console.log(model.CaseAssessment.CaseAssessmentRating.CaseAssessmentRatingID);
    ...
    this.GetAllInitialAssessmentProjects = function (callback, skip, take) {
        if (skip == undefined || take == undefined) {
            skip = 0;
            take = pagingSettings.pageSize;
        }
        $.ajax({
            url: "/InitialAssessmentQA/GetInitialAssessmentQACaseWorkflowPatientProjects/",
            cache: false,
            dataType: "json",
            data: { skip: skip, take: take },
            type: "POST",
            success: function (resultData) {
                self.InitaialAssessmentQAs([]);
                self.TotalItemCount(resultData.CaseWorkflowPatientReferrerProjectTotalCount);
                self.BindInitialAssessmentProjects(resultData);
                callback(true);
            }
        });
    };

    this.GetInitialAssessmentProjectsByTreatmentCategoryID = function (selectedTreatmentCategoryID, callback, skip, take) {
        if (skip == undefined || take == undefined) {
            skip = 0;
            take = pagingSettings.pageSize;
        }
        $.ajax({
            url: "/InitialAssessmentQA/GetInitialAssessmentQACaseWorkflowPatientProjectsByTreatmentCategoryID/",
            cache: false,
    ...
    this.BindInitialAssessmentProjects = function (data) {

      
        $.each(data.CaseWorkflowPatientReferrerProjects, function (index, value) {
           
            self.InitaialAssessmentQAs.push(new InitaialAssessmentQA(value.CaseID, value.PatientID, value.FirstName, value.LastName, value.ReferrerID, value.CaseNumber, value.CaseDateOfInquiry,
    ...
    this.CaseNumberClick = function (item) {
        ...
        window.location = "/InitialAssessmentQA/GetCaseAssessmentByCaseID/" + item.CaseID();
