function ExistingPatientsInitialAssessmentViewModel(config) {
    var self = this;
    ...
    self.loadGrid = function () {
        ...
        $.ajax({
            url: "/Supplier/ExistingPatientsInitialAssessment/GetInitialAssessments",
            type: 'post',
            dataType: 'json',
            success: function (result) {
                ...
                if (result.length == 0) {
                    self.Message(config.NoInitialAssessmentFound);
                    $('#grid-div').hide();
                } else {
                    $('#grid-div').show();
                }
    ...
    self.PrinBlankIA = function () {
        $.post('/PrintPopUp/PrintBlankInitialAssessment', { caseid: this.EncryptedCaseID }, function (resp) {
            var mywindow;
    ...
    self.Update = function (model) {
        if (model.IsCustom) 
            window.location.href = '/Supplier/InitialAssessmentCustom/Index/' + this.EncryptedCaseID;
        else
            window.location.href = '/Supplier/InitialAssessment/Index/' + this.EncryptedCaseID;
    };
