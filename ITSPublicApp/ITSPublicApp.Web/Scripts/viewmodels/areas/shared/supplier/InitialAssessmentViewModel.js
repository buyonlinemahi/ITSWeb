...
function InitialAssessmentViewModel() {
    ...
    self.PatientLevelOfRecoveries = ko.observableArray([self.PatientLevelOfRecoveries(0)]);
    self.selectedItemPatientLevelOfRecoveries = ko.observable(0);
    ...
    self.PreExistingConditionDescription = ko.observable();


    self.HasYellowFlags = ko.observable().extend({ changeToBool: null }).extend({ required: { message: '\n Is Patient demonstrate any yellow flags is required.' } });
    self.HasRedFlags = ko.observable().extend({ changeToBool: null }).extend({ required: { message: '\n Is Patient demonstrate any Red flags is required.' } });
    ...
    self.FactorsAffectingTreatmentDescription = ko.observable().extend({
        required: {
            onlyIf: function () {
    ...
    self.PatientLevelOfRecoveryID = ko.observable();
    ...
    self.Init = function (model) {        
        ...
        if (model.CaseAssessment != null) {
            ...
            if (model.CaseAssessment.WasPatientWorkingAtTheTimeOfTheAccident == null)
            {
               $("#WorkingNotApplicable").prop("checked",true);
            }
            ...
            {
                $("#ReturnNotApplicable").prop("checked",true);
            ...
            self.HasYellowFlags(model.CaseAssessment.HasYellowFlags);
            self.HasRedFlags(model.CaseAssessment.HasRedFlags);
            self.IsPatientUndergoingTreatment(model.CaseAssessment.IsPatientUndergoingTreatment);
            self.IsPatientTakingMedication(model.CaseAssessment.IsPatientTakingMedication);
    ...
    this.SubmitData = function () {
        var errors = "Errors ! Click on save Button before submit ";
        if (!self.isValid()) {
        ...
        if (this.CaseID() != null) {
            $.post('/InitialAssessment/Submit', { caseid: self.EncryptDecryptCaseID() }, function (resp) {
                window.location.href = '/Supplier/ExistingPatientsInitialAssessment';
            });
        }      
    };

    this.SaveInitialAssessmentform = function () {
    ...
    this.InitialAssessmentSuccess = function (responseText, statusText, xhr, $form) {
        window.location.href = '/Supplier/InitialAssessment/Index/' + this.EncryptDecryptCaseID();
        alert(responseText);
